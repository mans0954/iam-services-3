...
public class UserAttributeHelper {
    ...
    protected static final Log log = LogFactory
            .getLog(UserAttributeHelper.class);
    protected static LoginDataService loginManager;

    public static ExtensibleUser newUser(ProvisionUser pUser)
            ...
            for (Method m : userMethod) {
                ...
                if (methName.startsWith("get")) {
                    ...
                    if (returnType.contains("UserStatusEnum")) {
                        UserStatusEnum temp = (UserStatusEnum) m.invoke(pUser,
                                null);
                        if (temp != null) {
                    ...
                    // build the method name - to show the object that the value
                    // came from
                    // get rid of the "get" in the name
                    methName = "USER." + methName.substring(3);
                    extUser.getAttributes().add(
                            new ExtensibleAttribute(methName, value, 1,
                                    "String"));
                }
            ...
            for (Login lg : principalList) {
                try {
                    if (lg.getManagedSysId().equalsIgnoreCase("0")) {
                        extUser.getAttributes().add(
                                new ExtensibleAttribute("IDENTITY.PRINCIPAL.",
                                        lg.getLogin()));
                        extUser.getAttributes().add(
                                new ExtensibleAttribute(
                                        "IDENTITY_PSWD.PRINCIPAL", lg
                                                .getPassword()));
                        extUser.getAttributes().add(
                                new ExtensibleAttribute(
                                        "IDENTITY_STATUS.PRINCIPAL", lg
                                                .getStatus() == null ? "" : lg
                                                .getStatus().name()));
                    } else {
                        extUser.getAttributes().add(
                                new ExtensibleAttribute("IDENTITY."
                                        + lg.getManagedSysId(), lg.getLogin()));
                        extUser.getAttributes().add(
                                new ExtensibleAttribute("IDENTITY_PSWD."
                                        + lg.getManagedSysId(), lg
                                        .getPassword()));
                        extUser.getAttributes().add(
                                new ExtensibleAttribute("IDENTITY_STATUS."
                                        + lg.getManagedSysId(),
                                        lg.getStatus() == null ? "" : lg
                                                .getStatus().name()));
        ...
        /*
         * Login lg = getPrimaryPrincipal(principalList); if (lg != null) {
         * extUser.getAttributes().add(new
         * ExtensibleAttribute("PRINCIPAL.IDENTITY", lg.getLogin()));
         * extUser.getAttributes().add(new
         * ExtensibleAttribute("PRINCIPAL.IDENTITY_PSWD", lg.getPassword())); }
         */
        ...
        if (emailAddressList != null) {
            ...
            while (emailIt.hasNext()) {
                ...
                ExtensibleEmailAddress extEmail = new ExtensibleEmailAddress(
                        email);
    ...
    public static ExtensibleUser modifyUser(User origUser2,
            List<Role> curRoleList, List<Group> curGroupList,
            ProvisionUser pUser) throws IllegalArgumentException,
        ...
        for (UserAttribute attr : attrCol) {
            ...
            if (attr.getOperation() == (AttributeOperationEnum.DELETE)) {
                ...
                if (tempAttr != null) {
                    // log.info("Attribute found=" + name + " - val=" +
                    // tempAttr.getValue());
                    ...
                    if (attr.getValue() == null && tempAttr.getValue() != null) {
                        extAttr.setOperation(AttributeOperationEnum.REPLACE
                            ...
                            && tempAttr.getValue() == null) {
                        extAttr.setOperation(AttributeOperationEnum.REPLACE
                                .getValue());
                    } else if (!attr.getValue().equalsIgnoreCase(
                            tempAttr.getValue())) {
            ...
            for (Login lg : principalList) {
                try {
                    if (lg.getManagedSysId().equalsIgnoreCase("0")) {
                        ...
                        // extUser.getAttributes().add(new
                        // ExtensibleAttribute("IDENTITY_PSWD."+lg.getManagedSysId(),
                        // lg.getPassword(),0));
                        extUser.getAttributes().add(
                                new ExtensibleAttribute("IDENTITY_STATUS."
                                        + lg.getManagedSysId(), lg.getStatus()
                                        .name()));
                    }
                } catch (Exception e) {
                    log.error(e);
        ...
        Set<Address> addressList = getFullAddressList(pUser.getAddresses(),
                origUser2.getAddresses());
        // Set<Address> addressList = pUser.getAddresses();
        if (addressList != null) {
            Iterator<Address> adrIt = addressList.iterator();
            while (adrIt.hasNext()) {
        ...
        Set<EmailAddress> emailAddressList = getFullEmailList(
                pUser.getEmailAddresses(), origUser2.getEmailAddresses());
    ...
    static private Set<Address> getFullAddressList(Set<Address> newAddressList,
        ...
        if (origAddressList == null
                && (newAddressList != null && newAddressList.size() > 0)) {
            addressSet.addAll(newAddressList);
            return addressSet;
        }
        if ((origAddressList != null && origAddressList.size() > 0)
                && (newAddressList == null || newAddressList.size() == 0)) {
    ...
    static private Set<Phone> getFullPhoneList(Set<Phone> newPhoneList,
            Set<Phone> origPhoneList) {
        ...
        if ((origPhoneList != null && origPhoneList.size() > 0)
                && (newPhoneList == null || newPhoneList.size() == 0)) {
    ...
    static private Set<EmailAddress> getFullEmailList(
            Set<EmailAddress> newPhoneList, Set<EmailAddress> origEmailList) {
        ...
        if ((origEmailList != null && origEmailList.size() > 0)
                && (newPhoneList == null || newPhoneList.size() == 0)) {
    ...
    static public User cloneUser(User user) {
        ...
        //
        HashMap<String, UserAttribute> userAttributes = new HashMap<String, UserAttribute>(
                0);
    ...
    static private void determineUserChanges(ExtensibleUser extUser,
            User newUser, User origUser) {
        SimpleDateFormat df = new SimpleDateFormat(
                "EEE, dd MMM yyyy HH:mm:ss Z");
                ...
                try {
                    if (methName.startsWith("get")) {
                        if (returnType.contains("String")) {
                            ...
                            operation = getStringOperationValue(origUser,
                                    origUserClass, methName, value);
                        }
                        if (returnType.contains("Date")) {
                            ...
                            if (temp != null) {
                                ...
                                value = df.format(temp);
                                // value = temp.toString();
                                operation = getDateOperationValue(origUser,
                                        origUserClass, methName, time);

                            }
                        }
                        if (returnType.contains("UserStatusEnum")) {
                            UserStatusEnum temp = (UserStatusEnum) m.invoke(
                                    newUser, null);
                            if (temp != null) {

                                value = temp.getValue();
                                operation = getStatusOperationValue(origUser,
                                        origUserClass, methName, value);
                            }
                        }
                        // get the original value

                        // build the method name - to show the object that the
                        // value came from
                        // get rid of the "get" in the name
                        methName = "USER." + methName.substring(3);
                        extUser.getAttributes().add(
                                new ExtensibleAttribute(methName, value,
                                        operation, "String"));
                    }
                } catch (Exception it) {
                    it.printStackTrace();
    ...
    static private int getStatusOperationValue(User origUser,
            Class origUserClass, String methName, String value) {
        int operation = 0;
        try {
            Method method = origUserClass.getMethod(methName, null);
            UserStatusEnum origValue = (UserStatusEnum) method.invoke(origUser,
                    null);

            // log.info("operation check: method name=" + method + " value=" +
            // value + " " + origValue);
            ...
            if (origValue != null
                    && !origValue.getValue().equalsIgnoreCase(value)) {
                return 2;
            }
            return 0;
    ...
    static private int getDateOperationValue(User origUser,
            Class origUserClass, String methName, long newTime) {
        int operation = 0;
        long origTime = 0;
    ...
    static private int getStringOperationValue(User origUser,
            Class origUserClass, String methName, String value) {
        int operation = 0;
        try {
            Method method = origUserClass.getMethod(methName, null);
            ...
            // .info("operation check: method name=" + method + " value=" +
            // value + " " + origValue);
