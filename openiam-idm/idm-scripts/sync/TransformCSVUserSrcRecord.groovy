...
public class TransformCSVUserSrcRecord extends AbstractTransformScript {
	...
	public int execute(LineObject rowObj, ProvisionUser pUser) {
		...
		if (isNewUser) {
			...
			pUser.emailCredentialsToNewUsers = false;
			pUser.addInitialPasswordToHistory = true;
		}
		...
		if (isNewUser && pUser.status != UserStatusEnum.INACTIVE) {
			...
			for (String strR:  roleAry) {
				...
				if (!isInRole(userRoleList,  strR)) {
					...
					if (strR.equalsIgnoreCase("LOCAL_ADMIN")) {
						...
						println('Setting up user as delegated admin = ' + pUser.lastName);
