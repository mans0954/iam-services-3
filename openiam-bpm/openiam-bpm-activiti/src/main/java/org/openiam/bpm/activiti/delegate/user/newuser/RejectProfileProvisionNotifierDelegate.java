        ...
        for (final ApproverAssociationEntity approverAssociation : approverAssociationList) {
            ...
            switch(typeOfUserToNotify) {
            	case GROUP:
            		...
            		break;
            	case ROLE:
            		final List<String> usersInRole = userManager.getUserIdsInRole(notifyId, provisionRequestId);
					if(CollectionUtils.isNotEmpty(usersInRole)) {
						userIds.addAll(usersInRole);
					}
            		break;
            	case SUPERVISOR:
            		if(CollectionUtils.isNotEmpty(profileModel.getSupervisorIds())) {
            			userIds.addAll(profileModel.getSupervisorIds());
            		}
                    break;
            	case USER:
