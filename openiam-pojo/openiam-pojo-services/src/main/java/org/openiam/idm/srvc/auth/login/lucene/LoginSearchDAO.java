...
public class LoginSearchDAO extends AbstractHibernateSearchDao<LoginEntity, LoginSearchBean, String> {
	...
	protected Query parse(LoginSearchBean query) {
		...
		if(param != null && param.isValid()) {
			...
			switch(param.getMatchType()) {
				case EXACT:
					clause = buildExactClause("login", param.getParam());
					break;
				case STARTS_WITH:
					clause = buildTokenizedClause("login", param.getParam());
					break;
				default:
					break;
			}
	...
	public List<String> findUserIds(final int from, final int size, final LoginSearchBean query) {
    	...
    	if ((query != null)) {
            ...
            if (luceneQuery != null) {
				final List idList = findIds(buildFullTextSessionQuery(getFullTextSession(), luceneQuery, from, size, null).setProjection("userId"));
