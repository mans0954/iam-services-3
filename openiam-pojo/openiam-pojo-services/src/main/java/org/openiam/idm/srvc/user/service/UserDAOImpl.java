 ...
 */
@Repository("userDAO")
public class UserDAOImpl extends BaseDaoImpl<UserEntity, String> implements
		UserDAO {
	...
	public List<UserEntity> findByDelegationProperties(
		...
		if (search.getDelAdmin() == 1) {
			...
			if (search.getOrgFilter() != null) {
				...
				criteria.add(Restrictions.ilike("ua.value",
						search.getOrgFilter()));
			}
		}
	...
	public List<String> getUserIdList(int startPos, int count) {
		return (List<String>) getCriteria()
				.setProjection(Projections.property(getPKfieldName()))
				.setFirstResult(startPos).setMaxResults(count).list();
	}

	@Override
	public Long getUserCount() {
		return (Long) getCriteria().setProjection(
				Projections.count(getPKfieldName())).uniqueResult();
	}
	...
	public Long getUserCount(UserSearchBean searchBean) {
		return ((Number) getExampleCriteria(searchBean).setProjection(
				rowCount()).uniqueResult()).longValue();
	}
	...
	private Criterion getStringCriterion(String fieldName, String value,
		...
		if (StringUtils.isNotEmpty(value)) {
			if (matchMode != null) {
				...
				criterion = (caseInsensitive) ? Restrictions.eq(fieldName,
						value).ignoreCase() : Restrictions.eq(fieldName, value);
			}
		}
		return criterion;
	}
	...
	private Criteria getExampleCriteria(UserSearchBean searchBean) {
		boolean ORACLE_INSENSITIVE = "ORACLE_INSENSITIVE"
		...
		if (StringUtils.isNotBlank(searchBean.getKey())) {
			...
			if (searchBean.getShowInSearch() != null) {
				criteria.add(Restrictions.eq("showInSearch",
						searchBean.getShowInSearch()));
			}
			if (StringUtils.isNotEmpty(searchBean.getFirstName())) {
				criteria.add(getStringCriterion("firstName",
						searchBean.getFirstName(), ORACLE_INSENSITIVE));
			}
			if (StringUtils.isNotEmpty(searchBean.getLastName())) {
				criteria.add(getStringCriterion("lastName",
						searchBean.getLastName(), ORACLE_INSENSITIVE));
			}
			if (StringUtils.isNotEmpty(searchBean.getNickName())) {
				criteria.add(getStringCriterion("nickname",
						searchBean.getNickName()));
			}
			if (StringUtils.isNotEmpty(searchBean.getUserStatus())) {
				criteria.add(Restrictions.eq("status",
						UserStatusEnum.valueOf(searchBean.getUserStatus())));
			}
			if (StringUtils.isNotEmpty(searchBean.getAccountStatus())) {
				criteria.add(Restrictions.eq("secondaryStatus",
						UserStatusEnum.valueOf(searchBean.getAccountStatus())));
			}
			if (searchBean.getCreateDate() != null) {
				criteria.add(Restrictions.eq("createDate",
			...
			if (StringUtils.isNotEmpty(searchBean.getAttributeName())
					...
					|| StringUtils.isNotEmpty(searchBean
							.getAttributeElementId())
					|| (searchBean.getAttributeList() != null && !searchBean
							.getAttributeList().isEmpty())) {
				criteria.createAlias("userAttributes", "ua");
				if (searchBean.getAttributeList() != null
						&& !searchBean.getAttributeList().isEmpty()) {
				...
				if (StringUtils.isNotEmpty(searchBean.getAttributeName())) {
					criteria.add((ORACLE_INSENSITIVE) ? Restrictions.eq(
							"ua.name", searchBean.getAttributeName())
							.ignoreCase() : Restrictions.eq("ua.name",
							searchBean.getAttributeName()));
				}
				if (StringUtils.isNotEmpty(searchBean.getAttributeValue())) {
					criteria.add(getStringCriterion("ua.value",
							searchBean.getAttributeValue(), ORACLE_INSENSITIVE));
				}
				if (StringUtils.isNotEmpty(searchBean.getAttributeElementId())) {
					criteria.add(Restrictions.eq("ua.metadataElementId",
							searchBean.getAttributeElementId()));
				}
			}
			/* Login */
			if (StringUtils.isNotEmpty(searchBean.getPrincipal())
					|| StringUtils.isNotEmpty(searchBean.getDomainId())
					|| StringUtils.isNotEmpty(searchBean.getLoggedIn())) {
				criteria.createAlias("principalList", "lg");
				if (StringUtils.isNotEmpty(searchBean.getPrincipal())) {
					criteria.add(getStringCriterion("lg.login",
							searchBean.getPrincipal(), ORACLE_INSENSITIVE));
				}
				if (StringUtils.isNotEmpty(searchBean.getDomainId())) {
					criteria.add(Restrictions.eq("lg.domainId",
							searchBean.getDomainId()));
				}
	...
	private Criteria getUsersForResourceCriteria(final String resourceId) {
		return getCriteria().createAlias("resourceUsers", "ru").add(
				Restrictions.eq("ru.resourceId", resourceId));
	}

	@Override
	public List<UserEntity> getUsersForResource(final String resourceId,
	...
	private Criteria getUsersForGroupCriteria(final String groupId) {
		return getCriteria().createAlias("userGroups", "ug").add(
				Restrictions.eq("ug.grpId", groupId));
	}

	@Override
	public List<UserEntity> getUsersForGroup(final String groupId,
			final int from, final int size) {
	...
	private Criteria getUsersForRoleCriteria(final String roleId) {
		return getCriteria().createAlias("userRoles", "ur").add(
				Restrictions.eq("ur.roleId", roleId));
	}

	@Override
	public List<UserEntity> getUsersForRole(final String roleId,
			final int from, final int size) {
	...
	public void disassociateUsersFromOrganization(String organizationId) {
		final String queryString = String
				.format("UPDATE %s u SET u.organization = NULL WHERE u.organization.orgId = :organizationId",
	...
	public List<UserEntity> getUsersForMSys(final String mSysId) {
		Criteria criteria = getSession().createCriteria(UserEntity.class)
				.createAlias("principalList", "l")
				.add(Restrictions.eq("l.managedSysId", mSysId))
				.setFetchMode("principalList", FetchMode.JOIN);
		return criteria.list();
	}
}
