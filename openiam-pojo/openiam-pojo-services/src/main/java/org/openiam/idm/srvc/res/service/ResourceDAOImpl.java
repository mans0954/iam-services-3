...
public class ResourceDAOImpl extends OrderDaoImpl<ResourceEntity, String> implements ResourceDAO {
...
//            criteria.add(Restrictions.eq(getPKfieldName(), resource.getId()));
//        } else {
...
//                    criteria.createAlias("parentResources", "parent").add(Restrictions.in("parent.id", parentResourceIds));
//                }
//            }
//
...
//                    criteria.createAlias("childResources", "child").add(Restrictions.in("child.id", childResoruceIds));
//                }
//            }
...
//    }
    ...
    public List<ResourceEntity> getResourcesByType(String id) {
        Criteria criteria = getCriteria().createAlias("resourceType", "rt").add(Restrictions.eq("rt.id", id)).addOrder(Order.asc("displayOrder"));
    ...
    public List<ResourceEntity> getResourcesForGroup(final String groupId, final int from, final int size, final ResourceSearchBean searchBean) {
        final Criteria criteria = getCriteria().createAlias("groups", "rg").add(Restrictions.eq("rg.id", groupId)).addOrder(Order.asc("name"));
    ...
    public int getNumOfResourcesForGroup(final String groupId, final ResourceSearchBean searchBean) {
        final Criteria criteria = getCriteria().createAlias("groups", "rg").add(Restrictions.eq("rg.id", groupId)).setProjection(rowCount());
        addSearchBeanCriteria(criteria, searchBean);
        ...
        if (searchBean != null && criteria != null) {
            if (StringUtils.isNotBlank(searchBean.getKey())) {
                ...
                if (StringUtils.isNotEmpty(searchBean.getName())) {
                    String resourceName = searchBean.getName();
                    MatchMode matchMode = null;
                    if (StringUtils.indexOf(resourceName, "*") == 0) {
                        matchMode = MatchMode.END;
                        resourceName = resourceName.substring(1);
                ...
                if (CollectionUtils.isNotEmpty(searchBean.getAttributes())) {
                    ...
                    for (final Tuple<String, String> attribute : searchBean.getAttributes()) {
                        if (StringUtils.isNotBlank(attribute.getKey()) && StringUtils.isNotBlank(attribute.getValue())) {
                            criteria.add(Restrictions.and(Restrictions.eq("prop.name", attribute.getKey()), Restrictions.eq("prop.value", attribute.getValue())));
                ...
                if (CollectionUtils.isNotEmpty(searchBean.getChildIdSet())) {
                    criteria.createAlias("childResources", "childXrefs").createAlias("childXrefs.memberEntity", "child").add(Restrictions.in("child.id", searchBean.getChildIdSet()));
                }

                if (CollectionUtils.isNotEmpty(searchBean.getParentIdSet())) {
                    criteria.createAlias("parentResources", "parentXrefs").createAlias("parentXrefs.entity", "parent").add(Restrictions.in("parent.id", searchBean.getParentIdSet()));
                }

                if (CollectionUtils.isNotEmpty(searchBean.getRoleIdSet())) {
                    criteria.createAlias("roles", "roleXrefs").createAlias("roleXrefs.entity", "role").add(Restrictions.in("role.id", searchBean.getRoleIdSet()));
                }
