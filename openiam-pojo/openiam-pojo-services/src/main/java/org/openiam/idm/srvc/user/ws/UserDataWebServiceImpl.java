 ...
 */
@Service("userWS")
@WebService(endpointInterface = "org.openiam.idm.srvc.user.ws.UserDataWebService", targetNamespace = "urn:idm.openiam.org/srvc/user/service", serviceName = "UserDataWebService", portName = "UserDataWebServicePort")
public class UserDataWebServiceImpl implements UserDataWebService,
	...
	public Response addAddress(final Address val) {
		...
		try {
			...
			final AddressEntity entity = addressDozerConverter.convertToEntity(
					val, true);
	...
	public Response addAddressSet(Set<Address> adrList) {
		...
		try {
			...
			final List<AddressEntity> entityList = addressDozerConverter
					.convertToEntityList(new ArrayList<Address>(adrList), true);
	...
	public Response addAttribute(final UserAttribute attribute) {
		...
		try {
			...
			if (attribute.getName() == null
				...
				throw new BasicDataServiceException(
						ResponseCode.USER_ATTRIBUTE_NAME_NOT_SET);

			final UserAttributeEntity entity = userAttributeDozerConverter
	...
	public Response addEmailAddress(EmailAddress val) {
		...
		try {
			...
			final EmailAddressEntity entity = emailAddressDozerConverter
	...
	public Response addEmailAddressSet(final Set<EmailAddress> adrList) {
		...
		try {
			...
			String parentId = adrList.iterator().next().getParentId();
			final List<EmailAddressEntity> entityList = emailAddressDozerConverter
					.convertToEntityList(new ArrayList<EmailAddress>(adrList),
							true);
	...
	public Response addNote(final UserNote note) {
		...
		try {
			...
			final UserNoteEntity entity = userNoteDozerConverter
					.convertToEntity(note, true);
	...
	public Response addPhone(Phone val) {
		...
		try {
			...
			final PhoneEntity entity = phoneDozerConverter.convertToEntity(val,
					true);
			UserEntity user = new UserEntity();
			user.setUserId(val.getParentId());
			entity.setParent(user);
	...
	public Response addPhoneSet(final Set<Phone> phoneList) {
		...
		try {
			...
			final List<PhoneEntity> entityList = phoneDozerConverter
					.convertToEntityList(new ArrayList<Phone>(phoneList), true);
	...
	public Response addUser(User user) throws Exception {
		...
		try {
			...
			final UserEntity entity = userDozerConverter.convertToEntity(user,
					true);
	...
	@Override
	public Response addUserWithDependent(User user, boolean dependency)
			throws Exception {
		final Response response = new Response(ResponseStatus.SUCCESS);
		try {
			if (user == null) {
	...
	public List<User> findUserByOrganization(final String orgId) {
		final List<UserEntity> entityList = userManager
				.findUserByOrganization(orgId);
		return userDozerConverter.convertToDTOList(entityList, false);
	}

	@Override
	public List<User> findUsersByLastUpdateRange(final Date startDate,
			final Date endDate) {
		final List<UserEntity> entityList = userManager
				.findUsersByLastUpdateRange(startDate, endDate);
		return userDozerConverter.convertToDTOList(entityList, false);
	}
	...
	public List<User> findUsersByStatus(String status) {
		final List<UserEntity> entityList = userManager
				.findUsersByStatus(UserStatusEnum.valueOf(status));
		return userDozerConverter.convertToDTOList(entityList, false);
	}
	...
	@Override
	public List<Address> getAddressListByPage(String userId, Integer size,
			Integer from) {
	...
	public EmailAddress getEmailAddressById(String addressId) {
		final EmailAddressEntity adr = userManager
				.getEmailAddressById(addressId);
	...
	@Override
	public List<EmailAddress> getEmailAddressListByPage(String userId,
			Integer size, Integer from) {
		final List<EmailAddressEntity> adr = userManager.getEmailAddressList(
				userId, size, from);
	...
	public List<Supervisor> getEmployees(String supervisorId) {
		final List<SupervisorEntity> sup = userManager
	...
	@Override
	public List<Phone> getPhoneListByPage(String userId, Integer size,
			Integer from) {
		final List<PhoneEntity> phoneList = userManager.getPhoneList(userId,
				size, from);
	...
	public Supervisor getPrimarySupervisor(String employeeId) {
		final SupervisorEntity sup = userManager
				.getPrimarySupervisor(employeeId);
	...
	public User getUserByPrincipal(String securityDomain, String principal,
		...
		final UserEntity user = userManager.getUserByPrincipal(securityDomain,
				principal, managedSysId, dependants);
		return userDozerConverter.convertToDTO(user, dependants);
	}
	...
	public List<User> searchByDelegationProperties(
		...
		final List<UserEntity> userList = userManager
				.searchByDelegationProperties(search);
		return userDozerConverter.convertToDTOList(userList, false);

	}

	@Override
	public List<User> findBeans(
			@WebParam(name = "searchBean", targetNamespace = "") UserSearchBean userSearchBean,
			@WebParam(name = "from", targetNamespace = "") int from,
			@WebParam(name = "size", targetNamespace = "") int size) {
		final List<UserEntity> userList = userManager.findBeans(userSearchBean,
				from, size);
		return userDozerConverter.convertToDTOList(userList,
				userSearchBean.isDeepCopy());
	}
	...
	public Response updateAttribute(UserAttribute attribute) {
		...
		try {
			...
			final UserAttributeEntity entity = userAttributeDozerConverter
					.convertToEntity(attribute, false);
	...
	@Override
	public List<User> getUsersForResource(final String resourceId,
			final int from, final int size) {
		final List<UserEntity> entityList = userManager.getUsersForResource(
				resourceId, from, size);
		final List<User> userList = userDozerConverter.convertToDTOList(
				entityList, false);
		return userList;
	}
	...
	@Override
	public List<User> getUsersForGroup(final String groupId, final int from,
			final int size) {
		final List<UserEntity> entityList = userManager.getUsersForGroup(
				groupId, from, size);
		final List<User> userList = userDozerConverter.convertToDTOList(
				entityList, false);
		return userList;
	...
	@Override
	public List<User> getUsersForRole(final String roleId, final int from,
			final int size) {
		final List<UserEntity> entityList = userManager.getUsersForRole(roleId,
				from, size);
		final List<User> userList = userDozerConverter.convertToDTOList(
				entityList, false);
		return userList;
	...
	@Override
	public UserResponse saveUserInfo(final User user,
		...
		try {
			...
			final UserEntity userEntity = userDozerConverter.convertToEntity(
					user, true);
			SupervisorEntity supervisorEntity = null;
			if (supervisor != null)
				supervisorEntity = supervisorDozerConverter.convertToEntity(
						supervisor, true);
			String userId = userManager.saveUserInfo(userEntity,
					supervisorEntity);
	...
	@Override
	public Response enableDisableUser(final String userId,
			final UserStatusEnum secondaryStatus) {
		final Response response = new Response(ResponseStatus.SUCCESS);
		try {
			if (userId == null) {
				throw new BasicDataServiceException(
	...
	private void sendCredentialsToUser(User user, String identity,
			String password) throws BasicDataServiceException {
		...
		notificationRequest.getParamList().add(
				new NotificationParam("IDENTITY", identity));
		notificationRequest.getParamList().add(
				new NotificationParam("PSWD", password));
		notificationRequest.getParamList().add(
				new NotificationParam("USER_ID", user.getUserId()));
		final boolean sendEmailResult = mailService
	...
	public List<UserAttribute> getUserAttributes(final String userId) {
		...
		return (attributes != null) ? userAttributeDozerConverter
				.convertToDTOList(attributes, true) : null;
	}

	@Override
	public SaveTemplateProfileResponse saveUserProfile(
	...
	@Override
	public SaveTemplateProfileResponse createNewUserProfile(
		...
		try {
			...
			final CreateUserToken token = userProfileService
					.createNewUserProfile(request);
	...
	@Override
	public Response sendNewUserEmail(final String userId,
			final String password, final String login) {
		final Response response = new Response(ResponseStatus.SUCCESS);
		try {
			final NotificationRequest notificationRequest = new NotificationRequest();
			...
			notificationRequest.setNotificationType("NEW_USER_EMAIL");
			notificationRequest.getParamList().add(
					new NotificationParam("IDENTITY", login));
			notificationRequest.getParamList().add(
					new NotificationParam("PSWD", password));
			notificationRequest.getParamList().add(
					new NotificationParam("USER_ID", userId));
			final boolean sendEmailResult = mailService
					.sendNotification(notificationRequest);
			if (!sendEmailResult) {
				throw new BasicDataServiceException(
						ResponseCode.SEND_EMAIL_FAILED);
			}
		} catch (BasicDataServiceException e) {
			response.setErrorCode(e.getCode());
			response.setStatus(ResponseStatus.FAILURE);
	...
	public List<User> getByManagedSystem(String mSysId) {
		...
		return result == null ? new ArrayList<User>() : userDozerConverter
				.convertToDTOList(result, true);
	}
}
