...
public class GoogleLoginModule extends AbstractLoginModule {
    ...
    private static final Log log = LogFactory.getLog(GoogleLoginModule.class);
    ...
    protected void validate(AuthenticationContext context) throws Exception {
        ...
        GoogleProfile profile = jsonMapper.readValue(profileInfo, GoogleProfile.class);
        if(profile==null){
            newLoginEvent.setFailureReason("Invalid profile info");
            throw new BasicDataServiceException(ResponseCode.INVALID_PRINCIPAL);
        }
    ...
    protected LoginEntity getLogin(AuthenticationContext context) throws Exception {
        ...
        final String profileInfo = context.getSocialUserProfile();
        GoogleProfile profile = jsonMapper.readValue(profileInfo, GoogleProfile.class);
        String principal = profile.getEmails().get(0).getValue();
    ...
    private void userRegisteration(AuthenticationContext context, GoogleProfile profile, String principal) throws Exception {
        ...
        userEntity.setFirstName(profile.getName().getGivenName());
        userEntity.setLastName(profile.getName().getFamilyName());
        ...
        email.setEmailAddress(profile.getEmails().get(0).getValue());
    ...
    protected Subject doLogin(AuthenticationContext context, UserEntity user, LoginEntity login) throws Exception {
        //TODO: made a refactor after implemeting facebook login
        final String profileInfo = context.getSocialUserProfile();
        GoogleProfile profile = jsonMapper.readValue(profileInfo, GoogleProfile.class);
        String principal = profile.getEmails().get(0).getValue();
