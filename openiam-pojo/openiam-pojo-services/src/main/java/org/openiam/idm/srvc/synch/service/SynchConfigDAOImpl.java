    ...
    protected Criteria getExampleCriteria(SearchBean searchBean) {
        ...
        if (searchBean != null && searchBean instanceof SynchConfigSearchBean) {
            ...
            if (StringUtils.isNotBlank(sb.getName())) {
                criteria.add(Restrictions.ilike("name", sb.getName(), MatchMode.START));
            }
            if (StringUtils.isNotBlank(sb.getSynchType())) {
                criteria.add(Restrictions.eq("synchType", sb.getSynchType()));
            }
        }
        return criteria;
    }
