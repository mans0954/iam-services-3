...
public abstract class AbstractHibernateSearchDao<T, Q, KeyType> extends HibernateDaoSupport implements HibernateSearchDao<T, Q, KeyType>, DisposableBean, InitializingBean {
    ...
    @Override
    public List<T> find(final int from, final int size, final SortType sort, final Q query) {
    ...
    @Override
    public List<KeyType> findIds(final SortType sort, final Q query) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<KeyType> findIds(final int from, final int size, final SortType sort, final Q query) {
        final List<KeyType> result = new ArrayList<KeyType>();
        if ((from >= 0) && (size > 0) && (query != null)) {
            final Query luceneQuery = parse(query);
            if (luceneQuery != null) {
    ...
    protected Query buildTokenizedClauseMUST(final String paramName, final String paramValue) {
        if (StringUtils.isNotBlank(paramValue) && StringUtils.isNotBlank(paramName)) {
            final BooleanQuery paramsQuery = new BooleanQuery();
            paramsQuery.add(QueryBuilder.buildQueryMUST(paramName, paramValue), BooleanClause.Occur.SHOULD);
            return paramsQuery;
        }
        return null;
    }
    ...
    @Override
    public Date getLastDbUpdateDate() {
        return lastUpdateDBDate;
    }

    @Override
    public void synchronizeIndexes(final boolean forcePurgeAll) {
        reentrantLock.lock();
