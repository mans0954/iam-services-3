...
public class JDBCResoruceDAOImpl extends AbstractJDBCDao implements ResourceDAO  {
	...
	private String GET_ALL = "SELECT TYPE_ID AS TYPE_ID, RESOURCE_ID AS RESOURCE_ID, RESOURCE_TYPE_ID AS RESOURCE_TYPE_ID, NAME AS NAME, DESCRIPTION AS DESCRIPTION, IS_PUBLIC AS IS_PUBLIC, RISK AS RISK, ADMIN_RESOURCE_ID AS ADMIN_RESOURCE_ID, COORELATED_NAME AS COORELATED_NAME FROM %s.RES";
	private String GET_ALL_MENUS = "SELECT TYPE_ID AS TYPE_ID, RESOURCE_ID AS RESOURCE_ID, URL AS MENU_URL, NAME AS MENU_NAME, DISPLAY_ORDER AS DISPLAY_ORDER, IS_PUBLIC AS IS_PUBLIC, RISK AS RISK FROM %s.RES WHERE RESOURCE_TYPE_ID = ?";
	private String GET_AUTH_MENU_BY_ID = "SELECT TYPE_ID AS TYPE_ID, RESOURCE_ID AS RESOURCE_ID, URL AS MENU_URL, NAME AS MENU_NAME, DISPLAY_ORDER AS DISPLAY_ORDER, IS_PUBLIC AS IS_PUBLIC, RISK AS RISK FROM %s.RES WHERE RESOURCE_TYPE_ID = ? AND RESOURCE_ID = ?";
	...
	private static class ResourceMapper implements RowMapper<AuthorizationResource> {
		...
		public AuthorizationResource mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            resource.setCoorelatedName(rs.getString("COORELATED_NAME"));
			resource.setPublic("Y".equals(rs.getString("IS_PUBLIC")));
			resource.setMetadataTypeId(rs.getString("TYPE_ID"));
			return resource;
		}
